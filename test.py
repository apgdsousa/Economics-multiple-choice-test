...
def run_test(questions):
    ...
    valid_answers = ["A", "B", "C", "D"] #the user will only be allowed to use one of these letters
    for question in questions:# Loop through each question in the list of questions
        ...
        for option in question["options"]:# Loop through all the possible answer options for the question
            print(option)
        while True: #this loop will repeat until a valid answer is given 
            answer = input("Enter your answer (A, B, C, D): ").upper()
            if answer in valid_answers:
                break
            else:
                print("Invalid option. Please choose only A, B, C or D.")    
